...
$mes = $_POST['mes'];
$ano = $_POST['ano'];
$query = sprintf("select crm.idmovimento,crm.idcliente,pa.idpassagem, pa.embarque, pa.desembarque,c.nome as nomecliente, crm.valor_total, crm.qtdparcelas, fr.tipo, crm.data_movimento, pa.status_passagem, sum(cr.valor) as contaspagas, fr.tipo, crm.desconto as desconto, c.nome, v.dataviagem
...
where pa.status_passagem <> 4 and EXTRACT(year FROM crm.data_movimento) = $ano and EXTRACT(month FROM crm.data_movimento) = $mes and crm.idsituacao = 2
group by crm.idmovimento,crm.idcliente,pa.idpassagem, pa.embarque, pa.desembarque,c.nome, crm.valor_total, crm.qtdparcelas, fr.tipo, pa.status_passagem, crm.data_movimento, fr.tipo, crm.desconto, c.nome, v.dataviagem"); 
...
//------------------------------Lista de pagamentos --------------------------------  
$query_pagamentos = sprintf("select cpm.idmovipagar, cpm.data_movimento, cpm.valor, cpm.tipopg, cpm.idformapg, cpm.qtdparcelas, fp.tipo, cpm.tipopg, cpm.descricao, cpm.data_pg, sc.situacao, tp.descricao as tipo_despesa
...
where cpm.cancelado <> 1 and EXTRACT(year FROM cpm.data_movimento) = $ano and EXTRACT(month FROM cpm.data_movimento) = $mes and cpm.idsituacao = 2
...
// executa a query 
$dados_pagamentos = mysqli_query($con,$query_pagamentos) or die(mysqli_error($con)); 

// transforma os dados em um array 
$linha_pagamentos = mysqli_fetch_assoc($dados_pagamentos); 
    
$totalpg = mysqli_num_rows($dados_pagamentos);


//------------------------------Soma o valor de recebimentos pagos ---------------------
$somarecebpago = sprintf("select sum(cr.valor) as contaspagas
from contas_receb_movi crm 
inner join clientes c on crm.idcliente = c.idcliente 
left join contas_receber cr on cr.idmovimento = crm.idmovimento
inner join passagem pa on pa.idpassagem = crm.idpassagem 
inner join formapg fr on fr.idformapg = cr.idformapg
where pa.status_passagem <> 4 and crm.idsituacao = 2 and EXTRACT(year FROM cr.data_pg) = $ano and EXTRACT(month FROM cr.data_pg) = $mes"); 

// executa a query 
$dados_receb_pago = mysqli_query($con,$somarecebpago) or die(mysqli_error($con)); 
...
inner join situacao_caixa sc on sc.idsituacao = cpm.idsituacao
inner join formapg fp on fp.idformapg = cpm.idformapg
where cpm.idsituacao = 2 and cpm.cancelado <> 1 and EXTRACT(year FROM cpm.data_movimento) = $ano and EXTRACT(month FROM cpm.data_movimento) = $mes"); 
...
$somageral = sprintf("select SUM(crm.valor_total) as total_geral 
    from contas_receb_movi crm 
    inner join clientes c on crm.idcliente = c.idcliente 
...
?>
                ...
                <div class="alert alert-warning">
                    <center><h4><i class="fa fa-tasks"></i> Fluxo Caixa <?php echo '0'.$mess.'/'.$anoo?></h4></center>
                </div>
                <div class="card-group">
                    <div class="card">
                        <div class="card-body">
                                <form action="financeiro_fluxo_pesquisa.php" class="form-horizontal" method="post">
                                    <section>
                                        <div class="row">
                                            <div class="col-md-12">
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group row">
                                                    ...
                                                    <select class="custom-select form-control" name="mes" id="mes">
                                                        <?php 
                                                        $q = mysqli_query($con,"select distinct extract(MONTH from crm.data_movimento) as mes, extract(year from crm.data_movimento) as ano
                                                            from contas_receb_movi crm 
                                                            inner join clientes c on crm.idcliente = c.idcliente 
                                                            inner join passagem pa on pa.idpassagem = crm.idpassagem 
                                                            inner join viagem v on v.idviagem = pa.idviagem
                                                            inner join formapg fr on fr.idformapg = crm.idformapg
                                                            where pa.status_passagem <> 4") or die(mysqli_error($con));
                                                        if(mysqli_num_rows($q)){ 
                                                        //faz o loop para preencher o campo criado com os valores retornados na consulta 
                                                        while($line = mysqli_fetch_array($q)) 
                                                        { 
                                                        echo "<option value='".$line['mes']."'>".utf8_encode($line['mes'])."</option>";
                                                        } 
                                                        }
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group row">
                                                    ...
                                                    <select class="custom-select form-control" name="ano" id="ano">
                                                        <?php 
                                                        $q = mysqli_query($con,"select distinct extract(year from crm.data_movimento) as ano
                                                            from contas_receb_movi crm 
                                                            inner join clientes c on crm.idcliente = c.idcliente 
                                                            inner join passagem pa on pa.idpassagem = crm.idpassagem 
                                                            inner join viagem v on v.idviagem = pa.idviagem
                                                        ...
                                                        while($line = mysqli_fetch_array($q)) 
                                                        { 
                                                        echo "<option value='".$line['ano']."'>".utf8_encode($line['ano'])."</option>";
                                                        } 
                                                        }
                                                         else {//Caso não haja resultados 
                                                        echo "<option value='none'>Nenhum resultado encontrado!</option>"; 
