...
?>
 ...
 <script>
    ...
    // Função para criar uma poltrona
    function createSeatElement(poltrona, status) {
        const td = document.createElement('td');
        td.id = `seat-${poltrona}`;
        td.className = status === 'ocupada' ? 'ocupada' : status === 'em processo' ? 'em-processo' : 'livre';
        td.textContent = poltrona;

        td.addEventListener('click', function() {
            if (td.classList.contains('livre')) {
                marcarPoltronaComoProcesso(poltrona); // Função para marcar como em processo e abrir o modal
            }
        });
    ...
    // Função para marcar a poltrona como "em processo" e abrir o modal
    function marcarPoltronaComoProcesso(poltrona) {
        // Requisição AJAX para marcar a poltrona como "em processo"
        $.ajax({
            url: 'marcar_poltrona_ocupada.php',
            type: 'POST',
            data: { idpoltrona: poltrona, idviagem: idViagem },
            success: function(response) {
                if (response.status === 'sucesso') {
                    $(`#seat-${poltrona}`).removeClass('livre').addClass('em-processo'); // Marca a poltrona como "em processo"
                    $('#modalCadastroPassagem').modal('show'); // Abre o modal de cadastro
                    $('#poltrona').val(poltrona); // Define o número da poltrona no modal
                } else {
                    alert('A poltrona já está ocupada.');
                }
            },
            error: function() {
                alert('Erro ao marcar a poltrona como ocupada.');
            }
        });
    }

    // Carregar as poltronas e seus status ao carregar a página
    async function loadSeats() {
        const response = await fetch(`get_seat_status.php?idviagem=${idViagem}`);
        const data = await response.json();
        generateSeats(totalSeats, data.occupiedSeats); // Atualiza o mapa com base nas poltronas ocupadas
    }

    loadSeats(); // Inicializa o carregamento das poltronas
