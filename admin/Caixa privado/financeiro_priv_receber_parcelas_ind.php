...
$query = sprintf("select cr.idparcelas, fr.idformapg, crm.idmovimento, crm.idcliente,p.idpassagem, p.origem, c.nome as nomecliente, cr.valorpg, cr.parcela, cr.vencimento, p.destino, p.data_passagem, fr.tipo, c.passaporte
...
where cr.idcliente = $idcliente and cr.idsituacao = 1"); 
...
inner join contas_receber cr on cr.idmovimento = crm.idmovimento
inner join situacao_caixa sc on sc.idsituacao = cr.idsituacao
where cr.idcliente = $idcliente and cr.idsituacao = 2"); 
...
$somageral = sprintf("select SUM(cr.valor) as total_geral from contas_receb_movi crm inner join clientes c on crm.idcliente = c.idcliente inner join tratamento t on t.idtratamento = crm.idtratamento inner join contas_receber cr on cr.idmovimento = crm.idmovimento where cr.idcliente = $idcliente"); 
