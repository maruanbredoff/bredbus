...
where DATE_FORMAT(cr.data_pg, '%Y-%m-%d') = CURRENT_DATE() and c.idcontrato = $idcontrato
...
where DATE_FORMAT(cpm.data_movimento, '%Y-%m-%d') = CURRENT_DATE() and cpm.idsituacao = 2 and cpm.idcontrato = $idcontrato
...
//------------------------------ Agenda -----------------------------------
$dados_agenda = mysqli_query($con,"select id,title,start,end,resp_agenda,color,tipo from agenda where DATE_FORMAT(start, '%Y-%m-%d') = CURRENT_DATE() and idcontrato = $idcontrato") or die(mysqli_error($con)); 
...
where DATE_FORMAT(cr.data_pg, '%Y-%m-%d') = CURRENT_DATE() and c.idcontrato = $idcontrato") or die(mysqli_error($con)); 
...
where DATE_FORMAT(cpm.data_pg, '%Y-%m-%d') = CURRENT_DATE() and cpm.idcontrato = $idcontrato") or die(mysqli_error($con)); 
