...
const td = document.createElement('td');
            ...
            td.addEventListener('click', () => {
                if (td.classList.contains('livre')) {
                    marcarPoltronaEmProcesso(poltrona);
                }
            });
            return td;
        }

async function marcarPoltronaEmProcesso(poltrona) {
    try {
        // Verifica se a poltrona já está ocupada ou em processo
        const response = await fetch(`verificar_ocupacao.php?poltrona=${poltrona}&idviagem=${idviagem}`);
        const data = await response.json();

        console.log('Resposta do servidor:', data); // Log para depuração

        if (data.ocupada) {
            alert('A poltrona já está ocupada ou em processo.');
        } else {
            // Marcar a poltrona como "reservada" (em processo)
            const seatElement = document.getElementById(`seat-${poltrona}`);
            if (seatElement.classList.contains('livre')) {
                seatElement.classList.remove('livre');
                seatElement.classList.add('reservada');
                $('#poltrona').val(poltrona);
                $('#customerModal').show();
            } else {
                alert('A poltrona já está em processo ou ocupada.');
            }
        }
    } catch (error) {
        console.error('Erro ao verificar ocupação da poltrona:', error);
    }
}
...
async function atualizarPoltronasOcupadas() {
    try {
        ...
        console.log('Dados das poltronas ocupadas:', data); // Log para depuração
...
async function buscarClientes(termo) {
    ...
    try {
        ...
        data.clientes.forEach(cliente => {
            ...
            tr.innerHTML = `
                <td><button type="button" class="btn btn-primary selecionar-cliente" data-cliente='${JSON.stringify(cliente)}'>Selecionar</button></td>
                <td>${cliente.nome}</td>
        ...
        $(document).ready(function() {
            ...
            $('.close').on('click', function() {
                $('#customerModal').hide();
                $('.em-processo').removeClass('em-processo'); // Remove a classe de "em processo"
            });
        });
